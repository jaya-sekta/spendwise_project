...
@section('title', 'Tambah Kategori Bawaan')
...
@section('content')
    ...
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold text-gray-800">Tambah Kategori Bawaan</h1>
        <p class="text-gray-500 text-sm mt-1">Kategori ini akan otomatis muncul di semua akun user.</p>
    </div>
        ...
        <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <select name="category_type"
                        class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-amber-400 focus:border-amber-400 outline-none">
                    <option value="primary" {{ old('category_type') == 'primary' ? 'selected' : '' }}>Primer (kebutuhan pokok)</option>
                    <option value="consumptive" {{ old('category_type') == 'consumptive' ? 'selected' : '' }}>Konsumtif (gaya hidup)</option>
                </select>
                @error('category_type')
                    <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="monthly_limit" value="{{ old('monthly_limit') }}" min="0"
                       placeholder="500000"
                       class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-amber-400 focus:border-amber-400 outline-none">
