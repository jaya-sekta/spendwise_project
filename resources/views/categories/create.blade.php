    ...
    <form action="{{ route('categories.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input type="text" name="category_name" id="category_name" value="{{ old('category_name') }}" placeholder="Contoh: Makanan & Minuman" class="w-full pl-11 pr-4 py-3 rounded-xl border @error('category_name') border-red-300 ring-1 ring-red-300 @else border-gray-200 @enderror focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition text-sm">
            </div>
            @error('category_name')
        ...
        {{-- Tipe Kategori --}}
        <div>
            <label for="category_type" class="block text-sm font-semibold text-gray-700 mb-2">Tipe Kategori</label>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label class="relative flex p-4 rounded-xl border border-gray-200 cursor-pointer hover:bg-slate-50 transition focus-within:ring-2 focus-within:ring-blue-500/20">
                    <input type="radio" name="category_type" value="primary" {{ old('category_type', 'primary') === 'primary' ? 'checked' : '' }} class="mt-0.5 text-blue-600 focus:ring-blue-500 h-4 w-4 border-gray-300">
                    <span class="ml-3 flex flex-col">
                        <span class="block text-sm font-bold text-gray-800">Primer</span>
                        <span class="block text-xs text-gray-500 mt-0.5">Kebutuhan utama yang wajib dipenuhi (contoh: Kost, Makan, Tagihan).</span>
                    </span>
                </label>

                <label class="relative flex p-4 rounded-xl border border-gray-200 cursor-pointer hover:bg-slate-50 transition focus-within:ring-2 focus-within:ring-blue-500/20">
                    <input type="radio" name="category_type" value="consumptive" {{ old('category_type') === 'consumptive' ? 'checked' : '' }} class="mt-0.5 text-blue-600 focus:ring-blue-500 h-4 w-4 border-gray-300">
                    <span class="ml-3 flex flex-col">
                        <span class="block text-sm font-bold text-gray-800">Konsumtif</span>
                        <span class="block text-xs text-gray-500 mt-0.5">Keinginan atau hiburan tambahan (contoh: Ngopi, Bioskop, Belanja baju).</span>
                    </span>
                </label>
            </div>
            @error('category_type')
